...
In `/applications`, first persist the exact approved capability by typing `APPROVE <capabilityId>` and selecting its checkbox. Starting the inspection remains a separate deliberate owner action. Any future proposal uses `lever-real-inspection-v7`, form contract `lever-application-inspection-v1`, and exactly `[OPEN_AND_INSPECT_ONLY]`. Because adapter version is identity-bound, v7 starts a new deterministic capability family at version 1/null predecessor. The production v7 browser disables JavaScript before navigation, while its request route still aborts and terminates on any observed non-`GET`/`HEAD` request. Upload or fill requires different future code and authority. Final submission always requires immediate owner review of the frozen packet and a fresh, short-lived, one-use consent bound to the complete packet, destination, form, adapter, capability, document, answer, and disclosure snapshot. A lost or ambiguous response is terminal `OUTCOME_UNKNOWN`; never retry it automatically.
