...
Calibration is now derived from private label and pair outcomes rather than counts alone. A below-threshold set is `UNCALIBRATED`; a count-qualified set with missing/inconsistent evidence, failed label or pair performance, any failed/missing safety gate, or missing/mismatched owner approval is `CALIBRATION_PENDING`; only a fully passing, version-bound private assessment may become `CALIBRATED`. Additive migration `0006_r2_calibration_qualification.sql` stores only safe counts, agreement basis points, an opaque evidence digest, versions, approval metadata, and blocker codes. It does not store label/pair contents, and legacy count-only calibrated runs cannot authorize new calibrated evaluations.

Verified `CURRENT` and teaching/break/visa-period legal-hours assertions must now have an `asOf`/`validThrough` interval containing the evaluation date. Missing, partial, future, or expired bounds yield `REVIEW_REQUIRED` and never a legal-hours blocker. Explicit ordered `DATE_WINDOW` behavior is unchanged.

## PR #13 final hardening result - 2026-09-10

The 12-case fictional corpus is executable rather than score-labelled metadata. Every case builds a fictional candidate profile, R2A evidence/coverage, version bindings, and any commute or duplicate inputs; production `evaluateR2Eligibility`, `scoreR2JobFit`, cross-source comparison, and R2 duplicate persistence produce the actual results. The corpus stores expected eligibility, contribution direction/code, recommendation, duplicate state, and ordinal band but no expected scorer output. Independent reruns, real weight mutation, altered eligibility, stale binding, actual-scorer monotonicity/ablation/bounds, and review/ineligible recommendation regressions pass.

Scoring now requires a closed versioned calibration context bound to scorer, weight, corpus, state, and optional durable calibration run. The current context remains `UNCALIBRATED`; only a content-qualified, threshold/safety/approval-bound durable run can be stored as `CALIBRATED` without rewriting older evaluations. Evaluation identity now includes job, profile, evidence-contract, normalization, coverage, eligibility-engine, scorer, weight, and calibration-context versions. Repository and SQL checks independently reject recommended non-eligible, under-coverage, or materially unresolved rows.

Immutable migration `0005_r2_matching_quality_hardening.sql` transactionally rebuilds only the R2 evaluation table, count-checks the copy, preserves v4 IDs/data and dependent history, and adds calibration context/run identity. Migrations 0000-0004 remain unchanged. Synthetic fresh/v3/v4/v5, backup/restore, future-schema, integrity, FK, and history gates pass. The ignored private database was backed up at v4, migrated to v5 with all prior row counts preserved, and received one local-only reevaluation: two legacy evaluations and four job versions remain; the two R2 evaluation rows are immutable history with one current row; four documents remain stale and two packets invalidated. The current result remains `REVIEW_REQUIRED`, not recommended, 71% coverage, 3 material unknowns, 3 conditions, 0 conflicts, and `UNCALIBRATED` from 0 private labels across 0 families/statuses.

Duplicate discovery now reconstructs both sides from immutable source observations and their first bound job-version snapshot. Owner corrections cannot reinterpret an older candidate digest; new observations create new candidates, and decisions remain versioned. Queue decisions bind a deterministic digest of all duplicate evidence/decisions. Local packet creation requires an exact current `PREPARING` decision bound to the current job/profile/R2 evaluation/evidence/coverage/duplicate tuple; evaluation and duplicate changes stale the decision and invalidate any packet. Commute scoring consumes R2A `commute.distance` and `commute.duration` field evidence without unit conversion. Legal-hours blockers require a verified current/date-window/period applicability basis; unresolved temporal applicability reviews. Owner corrections move only an affected unknown family to `PARTIAL` while preserving unparsed spans.
...
6. `recommended=true` requires `ELIGIBLE`, the current job/evidence version, the current candidate-profile version, a current evaluation, zero unresolved material conflict, zero unresolved material conditional/unknown blocker, the approved minimum extraction coverage, and the score threshold; score alone is insufficient and can never clear `REVIEW_REQUIRED`.
...
| Location/work-type/category fit uses unverified preference signals                        | `ALREADY_FIXED` | `scoreJobFit` gates preferred locations, preferred work types, and preferred categories through `preferences.signalVerification`. R2B adds current-version/evidence-state binding.                                                                                                                                                |
| Commute maximum preference can subtract fit points when unverified or unknown             | `FIX_IN_R2`     | `scoreJobFit` verification-gates the within-limit positive branch, but its fallback subtracts points for any non-null commute even when `transport.maximumCommuteKm` is `USER_CONFIRMATION_REQUIRED` or `UNKNOWN`. R2B requires those states to contribute zero positive and zero negative points.                                |
| `REVIEW_REQUIRED` jobs can currently become recommended                                   | `FIX_IN_R2`     | `scoreJobFit` currently sets `recommended` for every status except `INELIGIBLE` when the score reaches the threshold. R2B requires current `ELIGIBLE` status plus current job/evidence, profile, and evaluation versions, no unresolved material conflict or conditional/unknown blocker, approved coverage, and score threshold. |
...
Audit total: **21 matching defects** — 16 `FIX_IN_R2`, 5 `ALREADY_FIXED`, and 0 deferred from the required defect set. The two non-blocking PR #10 review carryovers below are separate and both `DEFER` to their correct trains.
