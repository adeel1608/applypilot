...
R2B binds every positive contribution to a current verified candidate fact and current supported job evidence. `UNKNOWN`, unverified, stale, unresolved conditional, and conflicting evidence yields no positive points and remains a visible coverage gap. Legal status and sensitive/protected identity fields never contribute. Recommendation requires current `ELIGIBLE` status, sufficient extraction coverage, and no unresolved conflict; it is not `score >= threshold` alone.
