...
Superseding R2 status: PR #12 is merged; R2A is `READY`, R2B and R2C are `READY`, and R2D software is `READY` on `feat/r2-matching-quality` pending exact-head PR review. Additive migration 0004 is implemented and the ignored database is schema v4 with clean integrity/FKs after verified backup/migration and one append-only private evaluation. Calibration remains `UNCALIBRATED`; source and runner authority remain unchanged.
