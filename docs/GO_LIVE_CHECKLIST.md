...
R2A 3.1.0 is merged and `READY`. R2B/R2C/R2D software is hardened on unmerged PR #13 and awaits final exact-head CI/human review. Migration 0005 preserves the immutable 0000–0004 chain and the ignored database is schema v5 with zero pending migrations, integrity `PASS`, and zero FK issues after verified v4 backup/migration and one local-only reevaluation. Calibration remains `UNCALIBRATED`; this does not expand Manual-intake Personal Beta authority or enable a source/runner.
...
3. R2 now provides typed normalization/evidence, class semantics, versioned uncalibrated scoring, immutable cross-source duplicate evidence, and queue freshness on unmerged PR #13. Ranking remains explicitly `UNCALIBRATED` until the private review threshold is met.
...
Superseding R2 status: PR #12 is merged; R2A is `READY`, R2B and R2C are `READY`, and R2D software is `READY` on `feat/r2-matching-quality` pending final PR #13 review. Hardening migration 0005 is implemented and the ignored database is schema v5 with clean integrity/FKs and preserved history. Executable golden cases use actual production engines; evaluation/calibration identity, immutable duplicate evidence, explicit current PREPARING, commute fields, temporal legal hours, correction coverage, and persistence invariants are verified. Calibration remains `UNCALIBRATED`; source and runner authority remain unchanged.
...
- [x] R2D fictional golden corpus, truthful private aggregate calibration state, uncertainty UX, and local validation pass. Exact-head PR CI remains pending; the 30-job private calibration threshold is intentionally unmet.
...
| Work rights/legal hours                | U/I/B/M   | Valid != unrestricted; expiry/hours/conditions and time basis explicit                                                        | Exact-unit and explicit temporal-applicability R2B regressions pass                       |
...
| Fit/recommendation                     | U/I/B/M   | Current verified evidence only; deterministic/explainable; no blocker/review override; calibrated label                       | R2B/R2D software gates pass; private calibration threshold remains intentionally open     |
| Corrections and queue                  | U/I/B/M   | Immutable correction and invalidation; queue binds current versions and duplicate state                                       | Owner-correction coverage and exact CURRENT PREPARING binding/invalidation pass           |
