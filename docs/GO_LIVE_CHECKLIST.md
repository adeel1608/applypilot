...
| `R2 MATCHING QUALITY`          | Approved R2 plan and slices R2A–R2D                                                                                                                                                                      | Typed truth/modality, class semantics, verified-only scoring, durable dedup/queue, golden/calibration gates | `PLAN_ONLY`; not required to keep current manual Beta authority, required before mature ranking/source scaling |
...
- [ ] R2A evidence-state/modality schemas and conservative additive migration are reviewed and implemented.
