...
Remaining mechanics are to commit this coherent implementation and result record, rerun the release check on the clean committed head, push only `feat/personal-live-v1-enablement`, open one PR titled `feat: prepare Personal Live V1 enablement`, require exact-head push and pull-request GitHub Actions to pass, and stop with that PR open and unmerged.
