...
Remaining release work is procedural only: commit the coherent implementation and reality documentation, run the complete named final gate on the committed head, push the same branch, open one PR titled `feat: complete R2 matching quality`, and require exact-head GitHub Actions success. The implementation PR must remain open and unmerged for ChatGPT review. Source-enabled Personal Beta remains `WAITING_FOR_APPROVED_TENANT`; Personal Live V1 remains `NOT_READY`; the real runner remains `TARGET_APPROVAL_REQUIRED`.
