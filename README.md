...
R2A 3.1.0 is merged. R2B eligibility and verified-only scoring, R2C durable duplicate/queue/correction handling, and the R2D executable fictional calibration framework are hardened on `feat/r2-matching-quality` pending final review in unmerged PR #13. The real local database is schema v5 and privately verified with preserved history; calibration truthfully remains `UNCALIBRATED`. This adds no source tenant, real runner, employer-form, upload, submission, or deployment authority.
