...
- **Real employer interaction:** target approval required. Five owner-authorized read-only visits stopped safely before a valid inventory; the latest exposed and safely stopped under v4's now-corrected popup-declaration/attempt conflation. No candidate data, click, form change, upload, or submission occurred. The v5 Lever inspection lane is hardened offline for exact `OPEN_AND_INSPECT_ONLY` authority, observed popup events, and value-free destination diagnostics; it has no fill, upload, authentication, or submit API. No active capability authorizes another visit.
