...
- **Real employer interaction:** target approval required. Six owner-authorized read-only visits stopped safely before a valid inventory; visit 6 stopped as `UNSUPPORTED_CONTROL`, but its v5 evidence did not retain a subtype and cannot prove the exact cause. No candidate data, click, form change, upload, or submission occurred. The v6 Lever inspection lane is hardened offline to retain readable unfamiliar controls as explicit unsupported inventory fields while structural uncertainty stops with a fixed value-free cause. It has no fill, upload, authentication, or submit API. No active capability authorizes another visit.
