...
- **Real employer interaction:** target approval required. Seven owner-authorized read-only visits stopped safely before a valid inventory; visit 7 stopped as `UNSUPPORTED_CONTROL` with fixed value-free diagnostic `BLOCKED_WRITE_REQUEST`. No candidate data, click, form change, upload, or submission occurred. The v7 Lever inspection lane is hardened offline to use a scriptless fresh browser while retaining readable unfamiliar controls as explicit unsupported inventory fields and keeping any observed write request or structural uncertainty terminal. It has no fill, upload, authentication, or submit API. No active capability authorizes another visit.
