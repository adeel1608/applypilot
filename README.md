...
R2A 3.1.0 is merged. R2B eligibility and verified-only scoring, R2C durable duplicate/queue/correction handling, and the R2D fictional calibration framework are complete on `feat/r2-matching-quality` pending review in one unmerged PR. The real local database is schema v4 and privately verified; calibration truthfully remains `UNCALIBRATED`. This adds no source tenant, real runner, employer-form, upload, submission, or deployment authority.
