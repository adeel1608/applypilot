...
- **Real employer interaction:** target approval required. Four owner-authorized read-only visits stopped safely before a valid inventory; the latest stopped on `DESTINATION_CHANGED`, and no candidate data, click, form change, upload, or submission occurred. The v4 Lever inspection lane is hardened offline for exact `OPEN_AND_INSPECT_ONLY` authority and value-free destination diagnostics; it has no fill, upload, authentication, or submit API. No active capability authorizes another visit.
