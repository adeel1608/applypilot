...
- **Real employer interaction:** first real target validation is proven, while every later interaction still requires fresh exact target approval. After seven safe terminal read-only visits, visit 8 completed the v7 scriptless passive inspection and durably recorded a bounded value-free form inventory. Across all visits, candidate data, clicks, form changes, uploads, and submissions remained zero. The consumed authority is revoked; the lane has no fill, upload, authentication, or submit API, and no active capability authorizes another visit.
