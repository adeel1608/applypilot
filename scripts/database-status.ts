...
try {
  ...
  console.log(
    `DATABASE_STATUS present bytes=${statSync(databasePath).size} schema_version=${version} pending_migrations=${Math.max(0, 2 - version)} integrity=${integrity === "ok" ? "PASS" : "FAIL"} foreign_key_issues=${foreignKeyIssues}`,
  );
  if (integrity !== "ok" || foreignKeyIssues > 0 || version > 2) process.exitCode = 1;
} finally {
  sqlite.close();
}
